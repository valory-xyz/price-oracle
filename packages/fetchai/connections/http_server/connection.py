...
class HTTPChannel(BaseAsyncChannel):
    ...
    async def connect(self, loop: AbstractEventLoop) -> None:
        ...
        :param loop: asyncio event loop
        """
        self.logger.info(f"11111111111111111: {222}")
        self.logger.debug(f"11111111111111111: {222}")
        if self.is_stopped:
    ...
    async def _http_handler(self, http_request: BaseRequest) -> Response:
        ...
        """
        self.logger.info(f"11111111111111111: {http_request}")
        self.logger.debug(f"11111111111111111: {repr(http_request)}")
        ...
        try:
            ...
            self.logger.info(f"http server send envelope: {envelope}")
